...
def _goal_material_engine() -> WorkflowAuthoringEngine:
    ...
    异常：目录实体不满足稳定身份或连接点合同（Handle Contract）时，由
    ...
    # ``action_template`` 与 ``action_handles`` 模拟真实第 2 版动作合同投影；动作
    # 输入的数据来源是 ``goal``，不是旧测试夹具使用的 ``executor``。
...
def test_material_source_edge_survives_normalized_python_round_trip() -> None:
    ...
    # ``compiled`` 是初始可信源码产生的候选图（Candidate Graph）与规范源码。
    ...
    # ``repeated`` 是规范源码在原候选图基线上重新编译的固定点证明。
