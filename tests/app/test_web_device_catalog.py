...
def test_get_authoring_device_catalog_connects_inventory_material_identity(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """创作设备目录必须把库存解析器接入投影。

    参数：``monkeypatch`` 隔离 Host、注册表（Registry）和库存组合根。返回：无。
    断言：``GET /authoring/device-catalog`` 的处理函数输出库存权威提供的
    实际设备 Material UUID，而不是资源树运行时 UUID。异常：无。
    """
    ...
    response = web_api.get_authoring_device_catalog()
