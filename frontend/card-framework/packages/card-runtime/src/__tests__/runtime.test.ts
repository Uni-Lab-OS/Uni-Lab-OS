...
import type { DataAdapter, DeviceSchema, DataSnapshot, Subscription } from '@labos/card-sdk'
...
function makeAdapter(schema: DeviceSchema) {
  ...
  const adapter = {
    ...
    subscribeState: vi.fn((_d: string, _k: string[], onData: (s: DataSnapshot) => void): Subscription => {
      push = onData
      return { close: vi.fn() }
    }),
...
describe('createRuntime / createContext', () => {
  it('createContext 注入 schema', async () => {
    ...
    const ctx = await rt.createContext({ deviceId: 'pump_1', mountEl: el, config: {} })
  ...
  it('subscribe→onData 收到快照', async () => {
    ...
    const rt = createRuntime(adapter)
    const ctx = await rt.createContext({ deviceId: 'pump_1', mountEl: {} as HTMLElement, config: {} })
    const seen = vi.fn()
    ctx.onData(seen)
    ctx.subscribe('pump_1', ['pressure'])
    const snap: DataSnapshot = { deviceId: 'pump_1', latest: { pressure: { ts: 1, value: 1.2 } }, series: {} }
    emit(snap)
  ...
  it('callAction 透传到 adapter', async () => {
    ...
    expect(adapter.callAction).toHaveBeenCalledWith('pump_1', 'aspirate', { volume: 100 })
  })
