...
describe("SubscriptionManager", () => {
  ...
  it("底层数据 fan-out 给所有监听者", () => {
    ...
    const call = subscribeState.mock.calls[0];
    if (call === undefined) throw new Error("subscribeState was not called");
    const onData = call[2] as (s: DataSnapshot) => void;
