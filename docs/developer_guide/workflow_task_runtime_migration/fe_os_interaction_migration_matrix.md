...
- **D1A-S1 单 Action 正式 Task/Job 已形成跨仓候选**：Core #162 冻结
  `POST/GET /api/v1/device-action-tasks`、固定具体设备、system-owned 单节点 source
  与前端 wire 不可见边界。OS `935c662c2a7bf2657338ee713881cabde97beecc`
  ...
  durable pending、feedback、typed result、cancel/restart；FE
  `1b6b8dcd0a73d50146d37a2e04e08af036b188d7` 最大限度复用原 DevicePanel、Action
  卡、参数/draft、锁/解锁和旧 execution event/result panel，只新增严格 service/controller、
  A1 identity join、S1 scope/capability 门和串行 feedback cursor 补读。真实 local OS
  浏览器 E2E 1/1，8 张截图，两个浏览器点击各创建一个 Task/Job，feedback 补读 4 次，
  browser error/WebSocket/旧 Run route/system source read 均为 0；当前仍是未 push、未
  Core pin、未 `stage:accepted` 的候选，状态归 Core #163、OS #16、FE #19；
...
| D1A-S1 设备页单 Action 正式 Task | OS `935c662c2a7bf2657338ee713881cabde97beecc` 新增 strict Backend-envelope `POST/GET /api/v1/device-action-tasks`；按 `(authority, template UUID)` 维护稳定、不可编辑、普通路由不可见的 `system/device-console` source，在同一事务 materialize `run_mode=single_node` Task、唯一 Job、固定设备 assignment 和幂等记录，提交后才唤醒唯一 scheduler；复用生产 Action transport type、共享唯一 Action manager，完成 busy durable pending、pre-dispatch commit、HostNode feedback/typed result、regular Task cancel、restart replay/unknown fence | FE `1b6b8dcd0a73d50146d37a2e04e08af036b188d7` 新增 deny-by-default capability 和窄 `deviceActionTasks` service；继续用 `GET /api/v1/devices` 取 live instance、A1 Catalog 取 typed template identity，只做唯一 join。原 DevicePanel、Action 卡、参数控件/localStorage draft、锁/手动解锁和 `edge-device__execution` event/result UI 全部复用；HTTP 201 只显示 accepted，SSE 只 invalidates，REST rehydrate 串行增量补读 feedback，terminal 后权威刷新并允许再次运行 | Core #162 / OS #16 / FE #19 / Core #163。真实 local OS→浏览器 1/1：已有 holder 时第二 Task durable pending，随后 running feedback→typed result→Action free→再次点击第二个正式 Task 成功；8 图，2 次浏览器 POST 对应 2 个 Task，feedback GET 4，旧 Run/Runtime WS/前端直连 Edge/system source route/browser error 均 0。OS `tests/` 2363 passed/4 skipped；FE 242 tests、typecheck、web/desktop build 通过。候选未 push/未 Core pin，不等于 accepted |
...
| **D1A-S1 Device console single Action Task** | OS `935c662c2a7bf2657338ee713881cabde97beecc`：system source、原子 Task/Job/idempotency、fixed device、唯一 scheduler/lock owner、busy pending、feedback/result/cancel/restart 与 opaque public projection | FE `1b6b8dcd0a73d50146d37a2e04e08af036b188d7`：原 DevicePanel/UI 全量复用，只换 strict service/controller；live device+A1 unique join、capability/S1 scope、幂等提交、SSE invalidation、串行 cursor、旧 event/result panel、再次运行 | 生产 EdgeScheduler/JobExecutionBackend/HostNode 路径和 WebSocketClient `DeviceActionManager` 是同一锁 owner；测试 driver 只占物理设备边界，不进入产品前端 | Core #162、OS #16、FE #19、Core #163；本仓/FE implementation spec 已在代码前落位。真实 OS E2E 1/1、8 图、ledger 和 full SHA 已生成；待 issue report、push/merge/Core pin/Feishu 后才可 accepted |
