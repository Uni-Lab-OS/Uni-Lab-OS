...
| **D1A-S1 Device console single Action Task** | OS `305a5caceda42acfe0d835263cd2e51cc6fc497d`：稳定 system source revision、完整 Action transport/input/output frozen contract、原子 Task/Job/idempotency、fixed device、唯一 scheduler/lock owner、device-wide busy/unknown fence、frozen feedback/result、cancel、restart/unknown fence、late reconciliation、正式 Scheduler identity port、终态 mapping 释放、Claim 异常 rollback、重复 identity 无损预检与 opaque public projection | FE `a380bf81666509b8a5bfe7a7c84af43576828dd9`：原 DevicePanel/UI 全量复用，只换 strict service/controller；live device+A1 unique join、capability/S1 scope、幂等提交、专用 SSE invalidation、串行 cursor、旧 event/result panel、再次运行；Electron local runtime 启用 Edge scheduler | 生产 EdgeScheduler/JobExecutionBackend/HostNode 路径和 WebSocketClient `DeviceActionManager` 是同一锁 owner；D1A 不构造 legacy scheduler identity，内部转换隔离在 Scheduler；formal mapping 仅覆盖 active lifecycle，Claim hook 异常不会留下可调度 run，重复 Task identity 不触碰既有运行；生产先组合 authority、后挂 Edge stack时会反向绑定；测试 driver 只占物理设备边界，不进入产品前端 | Core #162、OS #16、FE #19、Core #163；本仓/FE implementation spec 已在代码前落位。最终真实 OS E2E 1/1、18.8 秒、8 图、ledger、full SHA 与 timeline/monitor 负向证据已在 final5 生成；待独立复核、issue report、push/merge/Core pin 和物理设备/Feishu 验收后才可 accepted |
