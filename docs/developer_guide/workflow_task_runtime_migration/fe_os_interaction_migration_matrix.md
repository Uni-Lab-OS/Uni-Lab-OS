...
- **设备 Action 锁检测与手动解锁已有受测候选**：OS `feat/device-manual-unlock`
  的候选为 `760a53d14b532228e461e59ffec388e2813df4ae`，
  ...
  `integration/fe-os-migration` 的实现与恢复测试候选分别为
  `c2410fb`、`a54058af3e878cadb2ebee5b4750ec297c44a7c1`，复用现有设备列表、
  动作目录和参数面板，增加锁标识、
...
| live 设备/动作目录、A1 Catalog 与单节点执行 | 当前 FastAPI composition root 直接从进程内唯一 `HostNode + WebSocketClient.DeviceActionManager` 投影 Backend-envelope `GET /api/v1/devices`（`device-catalog/v1`）；不恢复 local bridge、内部 HTTP proxy、第二个 manager、旧 `/workflow-node-templates` 或 `/api/v1/runtime/runs`。持久 typed Workflow Catalog/执行仍分别归 A1/D1 | FE `57cb9ada76dc00bddc7e0347cd9488f9647a9900` 未改写原设备 UI，只让 `LaboratoryService` 解包 Backend envelope 并保留 `id/deviceKey/namespace/name/online/actions/typeName/busy/schema`；health 继续使用 `/api/v1/health`。设备动作仍只预览参数，执行按钮 fail closed 并移交 WorkflowTask | 基于 OS `bebc2128fe5ab6fd525e3d7c0f28c35691cced86` 的真实 composition→浏览器 E2E 通过 1/1；浏览器目录与 health 均 200，旧 Run/node-template/前端直连 Edge WebSocket 为 0，console/page error 为 0。这是 live directory 读取与锁恢复 gate，不接受 A1 fingerprint/editor 或 D1 result commit |
| 设备 Action 锁检测与手动解锁 | OS 候选 `bebc2128fe5ab6fd525e3d7c0f28c35691cced86` 在唯一 manager 内完成完整 holder 投影、CAS、active/queue 快照隔离和永久 late-terminal fence；初次/提升 Job 的 `send_goal` 与解锁串行化，锁快照读取与上报同临界区，逐 Job physical cancel 异常隔离；HostNode/Action 缺失 fail closed，malformed body 使用 Backend error envelope，路由只挂载一次；保留普通 `failed→success` 终态纠正 | FE `57cb9ada76dc00bddc7e0347cd9488f9647a9900` 最大限度复用原设备列表、设备头、Action 卡片、锁面板和二次确认框；仅 service 解包 `GET /api/v1/devices`/command envelope 并接受 `unlocked`，成功后重新读取权威目录，不乐观改写为空闲；测试 fixture 不暴露给产品前端 | Core #160 / OS #13 / FE #18。独立 RED 为 `10 failed, 1 passed`；修复后 target `24 passed`、`tests/app` 340、全仓 `2342 passed, 4 skipped`。独占 FE 预览端口的真实 OS composition E2E 1/1：完整 holder→安全确认→精确 `force_unlock`→权威 free→新 holder 再占用；10 张截图与完整 setup/page ledger 归档于 FE，`/api/v1/runtime/runs*`、node-template、前端直连 Edge WebSocket 和 browser error 均为 0。候选仍待 exact-SHA review、push/合并/Core pin，不等于 `stage:accepted` |
...
| **L0 Live device directory adapter** | 当前 FastAPI composition root 直接组合 live `HostNode` 与唯一 `DeviceActionManager`，公开 Backend-envelope `GET /api/v1/devices`；不恢复 local bridge，只提供实时设备/动作/holder 投影，不提供持久 Workflow Catalog 或执行入口 | FE `57cb9ada76dc00bddc7e0347cd9488f9647a9900` 最大限度复用原设备 UI，仅替换 `LaboratoryService` envelope 适配；旧 `/workflow-node-templates` 读取退役，参数预览继续 fail closed | Edge 的 live communication client 持有唯一 Action queue/holder；设备在线、busy、完整 `currentJobId` 与 schema 均来自同进程实时事实 | Core #160 / OS #13 / FE #18；OS `bebc2128fe5ab6fd525e3d7c0f28c35691cced86` + FE 候选真实链路 1/1、10 图、完整网络账本。它是 A1 前的独立读取/锁恢复迁移，不解除 A1/R2/D1 依赖，也不恢复单 Action Run |
