...
| **R1B Durable runtime kernel** | 已完成并本地合入：受测/受审候选 `6cc9390`，merge `c540337d`；FIFO command、Task/Job 状态机、journal/outbox、feedback、unknown/reconcile、重启恢复、唯一 `workflow.runtime.changed`；不含 DAG/device | UI1A service port 候选 `5ca7cd2`、merge `1449b26` 已完成；UI1B `WorkflowTaskController` 仍须先连 SSE、读取一致 REST snapshot、重连/去重/补水且不消费 event patch | transport session 只是执行投影，不成为终态权威 | Core #150；OS delivery `deepmodeling/Uni-Lab-OS#303` 已本地合入；FE umbrella `Uni-Lab-OS/uni-lab-fe#2`、UI1A `#3`、UI1B `#4`、UI1C `#5`、UI1D `#6` 和 Core integration gate `Uni-Lab-OS/Uni-Lab-Core#152` 已建立；真实 gate 待执行 |
