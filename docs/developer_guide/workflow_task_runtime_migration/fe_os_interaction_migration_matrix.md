...
| 已发布工作流（PublishedWorkflow）目录代际与编写转换 | OS 候选 `de5a9b9b` 让 `POST /api/v1/authoring/{compile,generate-python,validate}` 每次请求读取工作流服务（WorkflowService）的当前编译器（Compiler），修复子工作流发布后仍使用启动期旧目录指纹的问题；不新增路由或兼容端点 | FE `dev@5046e39` 在服务边界显式适配 v1/v2 类型化动作（Action）合同、JSON 字符串已发布工作流模式和空元数据动作就绪（ready）连接点；持久画布不再把原生 `ready` 投影成普通结构端口 | SZLab `f9a4b6d` 的 `single_sample_atomic_material.py` 嵌套标准物料转运：源图 72 节点/57 边，画布 30 可见节点/28 物料边/7 `ready` 边/11 转运节点，反向生成零诊断；真实 ROS2 只读门 19 主题、9 在线设备、132 物料节点，10 图，浏览器与 HTTP 错误 0。OS 精确目标 35 项、FE 360 项、SZLab 18 项通过；OS `dev-wt` 实现与未改基线均受相同 56 个收集错误阻塞，故当前仅为本地受测候选，不等于已接受 |
