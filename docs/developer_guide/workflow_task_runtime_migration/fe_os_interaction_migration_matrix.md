...
| 目标 `Uni-Lab-OS/uni-lab-fe` | FE-D117 候选 `c779d473a2553c07b5e0a8551649567085501c28`；UI1A production/test 候选 `5ca7cd2b2baa5d0656626af25874fd597b19c267`；最终文档交接 `27923be820d350fc6cbc5626ff872cdbe8a80a00`；当前 FE integration `c9951973f33c85166cd8048cdc5b40e0182ddef6` | FE-D117 Authoring 单写权威和 UI1A Runtime service port 已本地合入；UI1B controller、UI1C 产品 UI 与 UI1D 真实 OS E2E/旧接口退役尚未完成；未获授权 push |
...
- **前端已完成**：FE-D117 已关闭 Authoring 单写权威和真实浏览器 delivery gate；
  ...
  只承载 invalidation 的全局 SSE 重连、游标和去重；UI1A 不包含产品 UI；
- **前端待完成**：UI1B `WorkflowTaskController` 的 coherent rehydration，UI1C 产品
  Runtime UI，以及 UI1D 真实 OS Playwright 和旧 Run/socket/polling 调用退役；
- **联调待完成**：R1B 与 UI1A 已有本地候选 full SHA；Core integration spec 应在
  UI1D 开始前写入 `Uni-Lab-OS/Uni-Lab-Core#152`，但真实 HTTP/SSE gate 只在 FE
  UI1D 候选和 OS R1B 候选都固定后执行。此前不得把 Core Decision
...
|---|---|---|---|
| `POST/GET /api/v1/workflow-tasks` 与 `GET .../jobs` | 已按 Backend DTO 持久化 Task、snapshot、预创建 Job；02H 完成 input preflight | UI1A service port 已接入严格 envelope 和 UUID/query 路由；UI1B controller 尚未接入 | 待真实 FE→OS Task create/read gate |
| `POST .../workflow-tasks/{uuid}/commands` | R1A 完成 durable ingress；R1B 完成 FIFO consumption、result、pause/resume/step permit/cancel | UI1A service port 已返回 durable command record，且不乐观补写 Task；UI1B controller 尚未接入 | 待重放、terminal race 和 cancel rehydration E2E |
| `GET /api/v1/workflow-node-jobs/{uuid}/feedback` | R1B 完成 sequence cursor、双键幂等 history、summary 与 restart persistence | UI1A service port 已接入 Job 与 sequence-cursor feedback；UI1B 补读策略尚未接入 | 待分页、SSE invalidation 后补读 E2E |
| `GET /api/v1/events` 的 `workflow.runtime.changed` | R1B 完成同事务 outbox、全局 cursor/replay，payload 仅有 `workflow_task_uuid` | UI1A 已完成全局 SSE、`Last-Event-ID`、有效/无效事件 ID 去重和 payload 防状态补丁；UI1B coherent rehydration 尚未接入 | 待先连 SSE、再读 snapshot、partial-read failure 和 OS restart E2E |
...
| **R1B Durable runtime kernel** | 已完成并本地合入：受测/受审候选 `6cc9390`，merge `c540337d`；FIFO command、Task/Job 状态机、journal/outbox、feedback、unknown/reconcile、重启恢复、唯一 `workflow.runtime.changed`；不含 DAG/device | UI1A service port production/test 候选 `5ca7cd2`、最终文档交接 `27923be`、当前 integration `c9951973` 已完成；UI1B `WorkflowTaskController` 仍须先连 SSE、读取一致 REST snapshot、重连/去重/补水且不消费 event patch | transport session 只是执行投影，不成为终态权威 | Core #150；OS delivery `deepmodeling/Uni-Lab-OS#303` 已本地合入；FE umbrella `Uni-Lab-OS/uni-lab-fe#2`、UI1A `#3`、UI1B `#4`、UI1C `#5`、UI1D `#6` 和 Core integration gate `Uni-Lab-OS/Uni-Lab-Core#152` 已建立；真实 gate 待执行 |
...
| **UI1 前端纵向迁移** | 提供最终 Authoring、Catalog、Task/Job、Material 和 debug Interface | FE-D117 Authoring 已完成；Runtime 按 UI1A service port（已完成）→ UI1B controller → UI1C 产品 UI → UI1D 真实 OS E2E/旧接口退役迁移；Material/Debug 仍按其后端依赖分别进入 | 无 | 总览 `Uni-Lab-OS/uni-lab-fe#2`；实现票 `#3～#6`；UI1A spec 为 `docs/migration/workflow/ui1a-workflow-task-runtime-port.md`；HTTP/SSE integration gate 为 `Uni-Lab-OS/Uni-Lab-Core#152` |
...
M1 和 R1A/R1B 可以在 02H 后与其他不冲突切片并行；R1A/R1B OS delivery 和 UI1A
service port 已完成，下一 Runtime 顺序是 UI1B controller → UI1C 产品 UI → UI1D
真实 OS E2E/旧接口退役 → Core #152 HTTP/SSE 联调；R2 仍依赖 A1、I1、C1 和 M1
...
| R1、UI1 Runtime | 全局 Map `Uni-Lab-OS/Uni-Lab-Core#1`；功能目录 `Uni-Lab-OS/Uni-Lab-Core#130`；active Decision `Uni-Lab-OS/Uni-Lab-Core#150` | R1A OS child `deepmodeling/Uni-Lab-OS#302` 和 R1B OS child `deepmodeling/Uni-Lab-OS#303` 已本地合并；FE umbrella `Uni-Lab-OS/uni-lab-fe#2` 下 UI1A `#3` 已本地完成，UI1B `#4`、UI1C `#5`、UI1D `#6` 依次待办；Core HTTP/SSE integration gate `Uni-Lab-OS/Uni-Lab-Core#152` 已建立并等待两仓候选 |
