...
| Electron 本地领域 Edge 启动与 readiness | OS `c550da665906abaf9568365aaa1d990f4d092aa8` 让 Graph FQID、唯一 short alias 和 Package resource child 沿同一 PackageCatalog/Registry seam 激活；歧义 short alias 在 Material、Graph init 和 driver attach 全部 fail closed；不增加全局 `warehouse` 映射；已由 non-squash merge `815df9a29d9fd82c560bc44489ce0063267ff6d3` 发布 | FE `c1eef9fc3e7e4596f978c0e45ff8106eed1c2f02` 最大限度复用原 `ConnectionBar`、设备页和日志抽屉，只把 local-python HTTP/WS 与 Electron launcher 对齐到 `18003`，并在 health/templates/devices 都有效后 ready；产品前端不暴露 HostNode/内部 system source；已由 non-squash merge `2fae21724231b08a45cd7bb776f12fdfe1865586` 发布 | 已跟踪 SZLab Graph 的真实 Electron E2E 1/1：两次启动、两次停止，原设备 UI 两次显示 9 台设备；health 为 scheduler ready，device catalog 为 `device-catalog/v1`；10 图，ready 窗口 console/page error、HTTP 4xx/5xx 为 0。OS/FE integration 已 push；仍待 Core pin，不等于 Accepted |
