...
- **D1A-S1 单 Action 正式 Task/Job 已形成最终受测跨仓候选**：Core #162 冻结
  ...
  `4ca471bc4d02dcc6af12ad50851ae378fec396c0`
  ...
  late-result reconciliation；FE 最终候选
  `379f029622a7ccd7364947cf6e105ee1f27d4aee` 最大限度复用原 DevicePanel、Action
  ...
  和串行 feedback cursor 补读。按生产顺序先组合 Workflow authority、再挂 Edge stack 的
...
| **D1A-S1 Device console single Action Task** | OS `4ca471bc4d02dcc6af12ad50851ae378fec396c0`：稳定 system source revision、完整 Action transport/input/output frozen contract、原子 Task/Job/idempotency、fixed device、唯一 scheduler/lock owner、busy pending、frozen feedback/result、cancel、restart/unknown fence、late reconciliation 与 opaque public projection | FE `379f029622a7ccd7364947cf6e105ee1f27d4aee`：原 DevicePanel/UI 全量复用，只换 strict service/controller；live device+A1 unique join、capability/S1 scope、幂等提交、专用 SSE invalidation、串行 cursor、旧 event/result panel、再次运行 | 生产 EdgeScheduler/JobExecutionBackend/HostNode 路径和 WebSocketClient `DeviceActionManager` 是同一锁 owner；生产先组合 authority、后挂 Edge stack时会反向绑定；测试 driver 只占物理设备边界，不进入产品前端 | Core #162、OS #16、FE #19、Core #163；本仓/FE implementation spec 已在代码前落位。最终真实 OS E2E 1/1、8 图、ledger、full SHA 与 timeline/monitor 负向证据已生成；待独立复核、issue report、push/merge/Core pin 和物理设备/Feishu 验收后才可 accepted |
