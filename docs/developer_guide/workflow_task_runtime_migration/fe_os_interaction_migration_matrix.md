...
| 发布 `Uni-Lab-OS/uni-lab-fe` | FE-D117 候选 `c779d473a2553c07b5e0a8551649567085501c28`；UI1A production/test 候选 `5ca7cd2b2baa5d0656626af25874fd597b19c267`；UI1B 纠偏候选 `e864e491463191473ab4f691cc7c26a1c5d4c6e3`；UI1C 候选 `eb5e2a30b391a5c7aae7400bf616bcdfa0175065`；UI1D 受测/受审候选 `b2547ee51fd9d7e6f9f277c407e3503c59cc4085`；UI1E 候选 `95c072077b193cafaeb3c6d4977c10f22af911fb` | UI1D 已删除旧 Run DTO/client/hook、Runtime WebSocket、polling fallback、前端假运行权威和 local bridge E2E；随后复用原设备 UI 切换 live device/action directory；UI1E 在唯一持久 Authoring 面板中复用原文件选择、CodeMirror、DAG 和完整 diff UI，恢复 Python/JSON 导入而不恢复旧 Run/WS/轮询。交付状态分别见 `Uni-Lab-OS/uni-lab-fe#10` 和 `Uni-Lab-OS/uni-lab-fe#11` |
...
- **live 设备目录读取已迁移**：FE integration
  `95c072077b193cafaeb3c6d4977c10f22af911fb` 保留原
  `LaboratoryService` 和设备 UI，仅替换读取适配并把 health route 对齐到
  `GET /api/v1/health`；原设备列表、动作卡片、状态展示与
...
- **设备 Action 锁检测与手动解锁已有 current-composition 受测候选**：OS
  `migration/device-manual-unlock-no-local-bridge` 的代码候选为
  ...
  `migration/device-manual-unlock-envelope` 的候选为
  `dd95d8f90161f504ee060f68a04a639f67b9e0c8`，复用现有设备列表、
  动作目录和参数面板，增加锁标识、
  holder 展示与必须确认物理安全的解锁对话框。能力仅在 local-python Edge 启用，
  holder 缺失时 fail closed，成功后必须重新 GET 目录，不做乐观解锁；
- **D1A-S1 单 Action 正式 Task/Job 已形成最终受测跨仓候选**：Core #162 冻结
  `POST/GET /api/v1/device-action-tasks`、固定具体设备、system-owned 单节点 source
  与前端 wire 不可见边界。OS 实现候选
  `2c65cd139605985047d5ecf82592c8850636cbd9`（D1A 代码提交
  `305a5caceda42acfe0d835263cd2e51cc6fc497d`）
  以稳定 `system/device-console` Workflow/Node materialize 正式 Task/Job，并复用唯一
  EdgeScheduler、JobExecutionBackend、HostNode 和 WebSocketClient Action 锁 owner 完成
  durable pending、feedback、typed result、cancel/restart、transport unknown fence 和
  late-result reconciliation；claimed/transport-unknown 期间以 device-wide key 阻止同设备
  其他 Action 穿透，D1A 仅依赖 Scheduler 的正式 Task/Job identity port，不构造 legacy
  scheduler identity；formal mapping 在 success/failure/cancel 终态释放，Claim hook
  异常会同时回滚 mapping 与 Scheduler run，后续 reschedule 不得绕过 durable Claim；
  重复 Task identity 在任何写入前拒绝，不会误删既有 run/inflight/locks；FE 最终候选
  `b502e4379250f7b5d3b3ed7506d4476b50353538`（D1A 代码提交
  `a380bf81666509b8a5bfe7a7c84af43576828dd9`）最大限度复用原 DevicePanel、Action
  卡、参数/draft、锁/解锁和旧 execution event/result panel，只新增严格 service/controller、
  A1 identity join、S1 scope/capability 门、专用 `device_action_task.changed` invalidation
  和串行 feedback cursor 补读，并让 Electron local runtime 启动参数显式包含
  `--edge_scheduler`。按生产顺序先组合 Workflow authority、再挂 Edge stack 的
  真实 local OS 浏览器 E2E 1/1，8 张截图，两个浏览器点击各创建一个 Task/Job，
  `/timeline` 与 `/monitor/snapshot` 只投影公开 Job UUID；browser error、WebSocket、
  旧 Run route 和 system source read 均为 0。软件已通过 exact-SHA review；D1A
  production merge 为私有 OS `dev@f4d9c1e4`（包含 OS integration `25e71d1`）与 FE
  integration `b963a33d`。当前 FE integration/Core pin `0bf83ea` 包含该 D1A merge；
  最新 Core gitlink 以 Core integration branch 与 Wayfinder #1 为准。用户已确认真实物理
  设备/ROS 不构成 D1A acceptance gate；当前只待 Feishu acceptance，
  Core #162/#163 为 `stage:testing`，OS #16 与 FE #19 已关闭；
- **Authoring 文件导入已迁移**：UI1E 不恢复旧 `WorkflowToolbar` 或文件
  ...
  `WorkflowAuthoringGraph` JSON 先由 OS `POST /api/v1/authoring/generate-python`
  生成 Python，接受完整 diff 后才写 Draft；Apply 仍只发 server-issued
  `candidate_hash`。Canonical v2/Cloud JSON 和跨 Workflow Graph 均 fail closed；
- **指定 integration 上的本地领域 Edge 启动已形成受测候选**：OS
  `migration/f006r1-local-debugger-startup@c550da665906abaf9568365aaa1d990f4d092aa8`
  从 `integration/workflow-task-runtime@5095948e49def24ac30d3db21a3fc758b4314efc`
  修复 canonical FQID/唯一 short alias/歧义 fail-closed 与 Package resource factory
  激活；FE `fix/local-debugger-integration-startup@c1eef9fc3e7e4596f978c0e45ff8106eed1c2f02`
  从 `integration/fe-os-migration@113e5519b71e2d7dd4f86de2bd36e7afd439f735`
  对齐 launcher endpoint `18003`，在 health/templates/devices 三个接口均 ready 后才
  宣告运行。真实 Electron 完成启动→9 台设备→停止→重启→9 台设备→再停止，1/1、
  10 图、ready 窗口 browser error 与 HTTP 4xx/5xx 为 0；OS 已 non-squash 发布到
  `integration/workflow-task-runtime@1918dda087fff08fe21e2ac8507938eb608e38a2`，FE 已发布到
  `integration/fe-os-migration@2fae21724231b08a45cd7bb776f12fdfe1865586`；仍待 Core pin，
  只属于 testing；
- **F007 Edge/runtime safety 已完成 non-squash integration merge 与合并态复验**：OS
  `migration/f007-edge-runtime-safety@77c05f8f37d56c7179b1c7024337a43bb14d749f`
  在不挂载 Package Workspace 时
  仍建立 `os-local` Graph Authority，并跳过需要 PackageCatalog projection 的
  ResourceTreeSet bootstrap；同时让普通 WorkflowTask cancel 真正下发到
  `JobExecutionBackend.request_cancel`，仅在物理 Job 终态后把 Task 结算为
  `canceled` 并释放同设备后续任务；completion 与 cancel sweep 使用同一
  settlement 临界区，无法确认 cancel 或 transport outcome 时进入
  `execution_unknown` 并保留设备围栏。独立 RED 测试提交为
  `daac505f707a7ab0dc1fdd4327754943953f757e`；OS 全仓
  `2569 passed, 4 skipped`。FE
  `feat/ui-optimization-szlab-material-display@4a5a941ff3bc15636b2cba95b6b3da98743e25f9`
  保留原设备页与本地启动器，
  修复 0 Action 设备的 null 状态判断；有领域包时 ready 门必须看到
  非 `host_node` Action，无领域包时使用 OS 内置配置并只等待
  `device-catalog/v1`；HostLink 使用专用 `18004`，日志去 ANSI 后按时间/
  级别/来源/正文渲染，停止前主动 abort health/device/Action catalog 请求；
  外部 backend 仍可显式连接，renderer reload 可从 main 的 ready snapshot 恢复。
  真实 Electron 有领域包与无领域包 `2/2`、0 Action 设备 `1/1`，全程不清空或
  过滤 browser errors，本轮共归档 19 张截图。最终 exact-SHA review 为
  Standards `0B/0NB`、Spec `0B/0NB`。OS behavior merge
  `336a075f16c92410c3a70eca0671c1b11a397f9b`、FE merge
  `8907e608ba58b1b0e4fb53d2e12d4514d422a966` 均为 non-squash 且无冲突；合并态
  OS 全仓 `2569 passed, 4 skipped`、FE `419 passed`、真实 Electron `2/2`、
  0 Action `1/1`。本轮随 integration 发布，仍待 Core pin/私有 OS `dev`，不等于
  accepted；
- **联调进度**：R1B、UI1A、UI1B 和 UI1C 候选均已进入已推送的 OS/FE integration
  ...
  产出 8 张截图与网络账本。UI1D 在 FE `b2547ee51fd9d7e6f9f277c407e3503c59cc4085`、
  OS `68c364e2e70c228a9598fbe1f4a10510602b51c2` 上通过 Authoring/UI1B/UI1C/final
  gate 真实 OS 回归 8/8；最终账本记录 50 个请求、50 个响应、9 张截图、旧路由 0、
  WebSocket 0、应用/page error 0，并验证 step command 重放、409 conflict、terminal
  race，以及终态后 3.5 秒内 Task list/detail、Jobs、feedback 的 timer-driven GET 为 0。
  普通 Task create 在调试配置存在时仍不携带 `start_node_id` 或 `breakpoints`。
  exact-SHA Standards/Spec review 均为 0B/0NB。Core integration spec 已在 UI1D
  production change 前写入 `Uni-Lab-OS/Uni-Lab-Core#152`；新的 Core gitlink、团队证据
  和 Feishu stage 对齐是通过 review 后的发布动作，其状态只在 #150/#152 维护。
...
| Action/Workflow 终止与设备锁释放 | F007 让普通 Workflow Worker 扫描 `canceling` Task，通过正式 dispatcher port 请求物理 cancel；Job 保持 `cancel_requested` 直到 HostNode 回传终态，终态后才结算 Task/释放锁/重排同设备后续任务；completion 与 cancel sweep 串行结算，无法确认 cancel/transport outcome 时进入 `execution_unknown` 安全围栏，不假释放 | 继续使用现有 Task command 与设备 Action 事件流，不新增前端 lock authority，不暴露 HostNode | 独立 RED `daac505f` 先复现“command 已消费但未调 dispatcher cancel”；最终确定性交错回归 4/4、OS 全仓 `2569 passed, 4 skipped`，exact review `0B/0NB`；OS non-squash integration merge `336a075f` 合并态全仓结果相同 |
...
| 已发布工作流（PublishedWorkflow）目录代际与编写转换 | OS 候选 `5d5d3a86` 让 `POST /api/v1/authoring/{compile,generate-python,validate}` 每次请求读取工作流服务（WorkflowService）的当前编译器（Compiler）；不同工作流（Workflow）的完整目录重建与代际替换串行化，无编译器时不挂载转换路由，运行期目录丢失稳定映射为 `template_catalog_unavailable`，其余编译器错误净化为 `internal_error`，不回传自定义消息；不新增路由或兼容端点 | FE `dev@5046e39` 在服务边界显式适配 v1/v2 类型化动作（Action）合同、JSON 字符串已发布工作流模式和空元数据动作就绪（ready）连接点；持久画布不再把原生 `ready` 投影成普通结构端口 | SZLab `f9a4b6d` 的 `single_sample_atomic_material.py` 嵌套标准物料转运：源图 72 节点/57 边，画布 30 可见节点/28 物料边/7 `ready` 边/11 转运节点，反向生成零诊断；真实 ROS2 只读门 19 主题、9 在线设备、132 物料节点，10 图，浏览器与 HTTP 错误 0。OS 精确目标 48 项、FE 360 项、SZLab 18 项通过；OS `dev-wt` 实现与未改基线均受相同 56 个收集错误阻塞，故当前仅为本地受测候选，不等于已接受 |
| live 设备/动作目录、A1 Catalog 与单节点执行 | 当前 FastAPI composition root 直接从进程内唯一 `HostNode + WebSocketClient.DeviceActionManager` 投影 Backend-envelope `GET /api/v1/devices`（`device-catalog/v1`）；不恢复 local bridge、内部 HTTP proxy、第二个 manager、旧 `/workflow-node-templates` 或 `/api/v1/runtime/runs`。持久 typed Workflow Catalog/执行仍分别归 A1/D1 | FE `57cb9ada76dc00bddc7e0347cd9488f9647a9900` 未改写原设备 UI，只让 `LaboratoryService` 解包 Backend envelope 并保留 `id/deviceKey/namespace/name/online/actions/typeName/busy/schema`；health 继续使用 `/api/v1/health`。设备动作仍只预览参数，执行按钮 fail closed 并移交 WorkflowTask | 基于 OS `da64ca2adeb9a0d7e5f89d75d52f45929ff68a7d` 与 FE `dd95d8f90161f504ee060f68a04a639f67b9e0c8` 的真实 composition→浏览器 E2E 通过 1/1；浏览器目录与 health 均 200，旧 Run/node-template/前端直连 Edge WebSocket 为 0，console/page error 为 0。这是 live directory 读取与锁恢复 gate，不接受 A1 fingerprint/editor 或 D1 result commit |
| Electron 本地领域 Edge 启动与 readiness | 既有 F006R1 继续负责 PackageCatalog/Graph FQID 激活；F007 OS `77c05f8f` 补齐无 Package Workspace 时的 `os-local` Graph Authority，且不执行缺少 PackageCatalog projection 的 ResourceTreeSet inventory bootstrap | FE `4a5a941` 最大限度复用原 `ConnectionBar`、设备页和日志抽屉。领域项目可选：有领域包时 ready 必须有非 HostNode Action；无领域包时使用 OS 内置 config，只等待合法 `device-catalog/v1`，不等待 Workflow Template Catalog；HTTP `18003`、HostLink `18004`；日志抽屉仅更换渲染层为结构化行并完整清除 ESC/C1 控制序列；停止前 abort health/device/Action catalog 请求，外部 backend 显式连接与 renderer reload 恢复不受托管 launcher 阻断 | 真实 Electron：SZLab 领域包 1/1（启动、9 设备及 Action、health、日志、reload 恢复、停止、重启）；纯 OS 无领域包 1/1（内置 config、HostNode catalog、格式化日志、停止）；0 Action 设备独立 1/1，点击与刷新均无 page error；全程 browser error 0，共 19 图。合并态 OS `336a075f` + FE `8907e608` 复验通过，随 integration 发布；仍待 Core pin，不等于 Accepted |
| 设备 Action 锁检测与手动解锁 | OS 候选 `da64ca2adeb9a0d7e5f89d75d52f45929ff68a7d` 在唯一 manager 内完成完整 holder 投影、CAS、active/queue 快照隔离和永久 late-terminal fence；初次/提升 Job dispatch、trace 延迟发送、ROS submit/accept 与解锁均线性化，unlock-first 不提交、submit-first 接受后立即取消；晚到 feedback/result 和旧 HTTP 结果仓不能覆盖 manual fence；锁快照读取与上报同临界区，逐 Job physical cancel 异常隔离；HostNode/Action 缺失 fail closed，malformed body 使用 Backend error envelope，路由只挂载一次；保留普通 `failed→success` 终态纠正 | FE service 候选 `57cb9ada76dc00bddc7e0347cd9488f9647a9900`、最终证据 HEAD `dd95d8f90161f504ee060f68a04a639f67b9e0c8`；最大限度复用原设备列表、设备头、Action 卡片、锁面板和二次确认框，原 `DevicePanel.tsx` 相对基线无 diff；仅 service 解包 `GET /api/v1/devices`/command envelope 并接受 `unlocked`，成功后重新读取权威目录，不乐观改写为空闲；测试 fixture 不暴露给产品前端 | Core #160 / OS #13 / FE #18。独立 RED 为 `10 failed, 1 passed`；修复后 target `29 passed`、trace `19 passed`、`tests/app` 345、全仓 `2347 passed, 4 skipped`。最终 OS 候选上的真实 composition E2E 1/1：完整 holder→安全确认→精确 `force_unlock`→权威 free→新 holder 再占用；10 张截图与完整 setup/page ledger 归档于 FE，`/api/v1/runtime/runs*`、node-template、前端直连 Edge WebSocket 和 browser error 均为 0。候选仍待 docs exact-SHA review、push/合并/Core pin，不等于 `stage:accepted` |
| D1A-S1 设备页单 Action 正式 Task | OS 对齐候选 `2c65cd139605985047d5ecf82592c8850636cbd9`（D1A 代码 `305a5caceda42acfe0d835263cd2e51cc6fc497d`）新增 strict Backend-envelope `POST/GET /api/v1/device-action-tasks`；按 `(authority, template UUID)` 维护稳定、不可编辑、普通路由不可见的 `system/device-console` source，在同一事务 materialize `run_mode=single_node` Task、唯一 Job、固定设备 assignment 和幂等记录，提交后才唤醒唯一 scheduler；A1 transport `template.type` 与 input/output contract 一起冻结进 revision/Task snapshot，旧 pending Task 不读取当前 Catalog；复用共享唯一 Action manager，完成 busy durable pending、pre-dispatch durable completion、HostNode feedback/frozen typed result、regular Task cancel、restart/transport-unknown fence、late result reconciliation；claimed/unknown fence 使用 device-wide key，D1A 通过 Scheduler 正式 identity port 提交/取消/回调，legacy identity 转换隔离在 Scheduler 内；formal mapping 在终态释放，Claim hook 异常原子回滚 mapping/run 且再次重排保持零派发；重复 Task identity 预检不会改动既有 run/inflight/resource locks/mapping；普通 Worker 排除 D1A Job，Scheduler timeline/monitor 只使用公开 Job UUID | FE 对齐候选 `b502e4379250f7b5d3b3ed7506d4476b50353538`（D1A 代码 `a380bf81666509b8a5bfe7a7c84af43576828dd9`）新增 deny-by-default capability 和窄 `deviceActionTasks` service；继续用 `GET /api/v1/devices` 取 live instance、A1 Catalog 取 typed template identity，只做唯一 join。原 DevicePanel、Action 卡、参数控件/localStorage draft、锁/手动解锁和 `edge-device__execution` event/result UI 全部复用；HTTP 201 只显示 accepted；全局 SSE 消费专用 `device_action_task.changed`，REST rehydrate 串行增量补读 feedback，terminal 后权威刷新并允许再次运行；Electron local runtime 明确以 `--edge_scheduler` 启动，与对外 capability 一致 | Core #162 / OS #16 / FE #19 / Core #163。按生产组合顺序的真实 local OS→浏览器 1/1：已有 holder 时第二 Task durable pending，随后 running feedback→typed result→Action free→再次点击第二个正式 Task 成功；8 图，2 次浏览器 POST 对应 2 个 Task，`/timeline`、`/monitor/snapshot` system source 字段为 0，旧 Run/Runtime WS/前端直连 Edge/system source route/browser error 均为 0。OS 全仓 `2347 passed, 4 skipped`，D1A direct target 30 passed；FE D1A 对齐候选六组 262 tests，当前 FE integration successor 286 tests，workspace typecheck、web/desktop build 均通过。D1A production merges 为私有 OS `dev@f4d9c1e4`（包含 OS integration `25e71d1`）与 FE integration `b963a33d`；当前 FE integration/Core pin `0bf83ea` 包含 D1A merge，最新 Core gitlink 以 Core integration branch/Wayfinder 为准；exact-SHA review 为 Standards `0B/1NB`、Spec `0B/0NB`。用户确认真实物理设备/ROS 不构成 acceptance gate；当前只待 Feishu acceptance，不等于 accepted |
| DAG readiness/admission 与 device result | 完整 R2/D1 尚未开始；R1B 本身不 dispatch。D1A-S1 仅是已冻结的“一个 typed Action + 固定具体设备 + 无 ResourceSlot/Material/Site”窄切片，不提供 DAG plan、自动设备分配或物料提交 | D1A-S1 可展示其专用 Task projection；完整 Workflow 仍只能展示 durable kernel projection，不得自行推进状态 | D1A-S1 证据不能替代 R2/D1 full gate；带 ResourceSlot/Material/Site 的 Action 继续 fail closed |
...
| **L0 Live device directory adapter** | 当前 FastAPI composition root 直接组合 live `HostNode` 与唯一 `DeviceActionManager`，公开 Backend-envelope `GET /api/v1/devices`；不恢复 local bridge，只提供实时设备/动作/holder 投影，不提供持久 Workflow Catalog 或执行入口 | FE `dd95d8f90161f504ee060f68a04a639f67b9e0c8` 最大限度复用原设备 UI，仅替换 `LaboratoryService` envelope 适配；旧 `/workflow-node-templates` 读取退役，参数预览继续 fail closed | Edge 的 live communication client 持有唯一 Action queue/holder；设备在线、busy、完整 `currentJobId` 与 schema 均来自同进程实时事实 | Core #160 / OS #13 / FE #18；OS `da64ca2adeb9a0d7e5f89d75d52f45929ff68a7d` + FE `dd95d8f90161f504ee060f68a04a639f67b9e0c8` 真实链路 1/1、10 图、完整网络账本。它是 A1 前的独立读取/锁恢复迁移，不解除 A1/R2/D1 依赖，也不恢复单 Action Run |
| **D1A-S1 Device console single Action Task** | OS aligned `2c65cd139605985047d5ecf82592c8850636cbd9`（D1A `305a5caceda42acfe0d835263cd2e51cc6fc497d`）：稳定 system source revision、完整 Action transport/input/output frozen contract、原子 Task/Job/idempotency、fixed device、唯一 scheduler/lock owner、device-wide busy/unknown fence、frozen feedback/result、cancel、restart/unknown fence、late reconciliation、正式 Scheduler identity port、终态 mapping 释放、Claim 异常 rollback、重复 identity 无损预检与 opaque public projection | FE aligned `b502e4379250f7b5d3b3ed7506d4476b50353538`（D1A `a380bf81666509b8a5bfe7a7c84af43576828dd9`）：原 DevicePanel/UI 全量复用，只换 strict service/controller；live device+A1 unique join、capability/S1 scope、幂等提交、专用 SSE invalidation、串行 cursor、旧 event/result panel、再次运行；Electron local runtime 启用 Edge scheduler | 生产 EdgeScheduler/JobExecutionBackend/HostNode 路径和 WebSocketClient `DeviceActionManager` 是同一锁 owner；D1A 不构造 legacy scheduler identity，内部转换隔离在 Scheduler；formal mapping 仅覆盖 active lifecycle，Claim hook 异常不会留下可调度 run，重复 Task identity 不触碰既有运行；生产先组合 authority、后挂 Edge stack时会反向绑定；测试 driver 只占物理设备边界，不进入产品前端 | Core #162、OS #16、FE #19、Core #163；D1A 已 non-squash 发布到私有 OS `dev@f4d9c1e4`（包含 OS integration `25e71d1`）和 FE merge `b963a33d`，当前 FE integration/Core pin `0bf83ea` 包含该 FE merge，最新 Core gitlink 以 Core integration branch/Wayfinder 为准；final6 E2E 1/1、18.8 秒、8 图、ledger、full SHA 与 timeline/monitor 负向证据均归档；OS #16、FE #19 已关闭。用户确认真实物理设备/ROS 不构成 acceptance gate；仅 Feishu acceptance 未完成，Core #162/#163 保持 `stage:testing`，不得 accepted |
| **UI1E Authoring 文件导入** | 无新增 OS Interface；复用 Workflow-scoped aggregate、`POST /api/v1/authoring/generate-python`、Draft PUT 与 single-token Apply | FE `95c072077b193cafaeb3c6d4977c10f22af911fb` 在唯一持久 Authoring 面板恢复 Python/JSON 导入；文件只进入当前 Workflow 脏态，不成为 authority | 无 | Core Decision `Uni-Lab-OS/Uni-Lab-Core#139`；FE delivery `Uni-Lab-OS/uni-lab-fe#11`；FE implementation spec `docs/migration/workflow/ui1e-authoring-file-import.md`；真实 OS E2E 3/3、9 图、3 账本，等待 Core pin/用户接受 |
...
| **R1B Durable runtime kernel** | 已完成并 non-squash 合入：受测/受审候选 `6cc9390`，merge `c540337d`；FIFO command、Task/Job 状态机、journal/outbox、feedback、unknown/reconcile、重启恢复、唯一 `workflow.runtime.changed`；不含 DAG/device | UI1A service port `5ca7cd2`、UI1B 纠偏 `e864e49`、UI1C resilience `eb5e2a3`、UI1D final candidate `b2547ee` 已完成，当前 FE integration `a641fa6`；原起点/断点/gutter、Feedback/Output/Debugger surface 均继续复用，旧 Run/WS/polling/前端假运行权威已删除 | transport session 只是执行投影，不成为终态权威 | Core #150/#152 已关闭；OS/FE integration、submodule pin、9 图、网络账本与 Feishu Accepted 均已发布；exact-SHA 双审 0B/0NB，真实 OS 8/8 |
...
| **X1 退役与接受** | OS 已删除 `/runtime/runs`、Run identity 和已取代 local bridge；其他 scheduler/plan 兼容清理由所属后续阶段负责 | UI1D 已删除 Run DTO/client/hook、旧 socket、轮询 fallback、旧 fallback panel、前端假运行权威和 local bridge E2E；静态门禁记录 160 个 production 文件、2 个 active fixture、19 个退役文件、0 个旧引用/轮询 fallback | 删除被新 plan/claim 合同取代的兼容锁或桥 | Core #152 记录各仓 full SHA、8/8 E2E、9 图、网络账本与 submodule pin；exact-SHA 双审已通过，Core/Feishu stage 对齐按 #150 执行 |
...
feedback/fault/restart hardening、UI1D 退役/final gate/exact-SHA review、Core #152
发布和 Feishu Accepted 均已完成；下一 Runtime production 阶段 R2 仍依赖 A1、I1、C1 和 M1
...
| R1、UI1 Runtime | 全局 Map `Uni-Lab-OS/Uni-Lab-Core#1`；功能目录 `Uni-Lab-OS/Uni-Lab-Core#130`；已接受 Decision `Uni-Lab-OS/Uni-Lab-Core#150` | R1A/R1B OS children、FE UI1A～UI1D umbrella/children 与 Core #152 均已关闭；真实 final gate 8/8、exact-SHA 双审 0B/0NB、submodule pin、9 图、网络账本与 Feishu Accepted 已发布 |
