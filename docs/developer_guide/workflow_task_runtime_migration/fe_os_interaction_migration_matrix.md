...
- **F007 Edge/runtime safety 已形成受测候选**：OS
  `migration/f007-edge-runtime-safety@6f08f895` 在不挂载 Package Workspace 时
  ...
  `canceled` 并释放同设备后续任务。独立 RED 测试提交为
  `daac505f`；候选 direct target `242 passed`，OS 全仓仅保留一个在
  本轮 base `25be15ef` 上可独立复现的既有 nullable ResourceSlot 失败。FE
  `feat/ui-optimization-szlab-material-display@8076380` 保留原设备页与本地启动器，
  ...
  级别/来源/正文渲染。真实 Electron 有领域包 1/1、无领域包 1/1、
  0 Action 设备 1/1，本轮共归档 18 张截图；仍待 exact-SHA review、
  push 和 Core Wayfinder 记录，不等于 accepted；
...
| Action/Workflow 终止与设备锁释放 | F007 让普通 Workflow Worker 扫描 `canceling` Task，通过正式 dispatcher port 请求物理 cancel；Job 保持 `cancel_requested` 直到 HostNode 回传终态，终态后才结算 Task/释放锁/重排同设备后续任务；无法确认 cancel 时进入 `execution_unknown` 安全围栏，不假释放 | 继续使用现有 Task command 与设备 Action 事件流，不新增前端 lock authority，不暴露 HostNode | 独立 RED 先复现“command 已消费但未调 dispatcher cancel”；修复后验证 cancel 下发、物理终态、Task `canceled`、同设备后续 Job 真正 dispatch，D1A 和普通 Workflow target 共 `242 passed` |
...
| Electron 本地领域 Edge 启动与 readiness | 既有 F006R1 继续负责 PackageCatalog/Graph FQID 激活；F007 OS `6f08f895` 补齐无 Package Workspace 时的 `os-local` Graph Authority，且不尝试执行缺少 PackageCatalog projection 的 ResourceTreeSet inventory bootstrap | FE `8076380` 最大限度复用原 `ConnectionBar`、设备页和日志抽屉。领域项目改为可选：有领域包时 ready 必须有非 HostNode Action，无领域包时使用 OS 内置 config 并只要有合法 catalog；HTTP `18003`、HostLink `18004`；日志抽屉仅更换渲染层为结构化行 | 真实 Electron：SZLab 领域包 1/1（启动、9 设备及 Action、health、日志、停止、重启）；纯 OS 无领域包 1/1（内置 config、HostNode catalog、格式化日志、停止）；0 Action 设备独立 1/1，点击与刷新均无 page error；共 18 图。候选待 push/Core pin，不等于 Accepted |
