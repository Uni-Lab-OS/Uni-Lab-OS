...
管理显式依赖、锁文件、安装事务、发布归档和云端广场兼容投影。它消费包目录
（PackageCatalog），但不决定工作区候选代或驱动激活。

## 公开 Interface

入口集中在 `__init__.py`：`PackageDependencyManager`、锁模型、安装与发布 Adapter。
软件包检查编排位于 `inspection.py`；其 `compile_catalog` 与上传 Adapter 的
`package_inspector` 必须由组合根显式注入。
