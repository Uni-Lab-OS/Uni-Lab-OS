...
def _locked_package_sort_key(package: LockedPackage) -> tuple[str, str]:
    """读取锁定外部包的规范排序键。
...
def _catalog_namespace(catalog: PackageCatalog) -> str:
...
def _package_source_namespace(package: PackageCatalogSource) -> str:
...
class PackageDependencyLock:
    ...
    def __post_init__(self) -> None:
        ...
        ordered = tuple(
            sorted(self.packages, key=_locked_package_sort_key)
        )
...
def load_locked_package_catalogs(
    ...
    return tuple(sorted(catalogs, key=_catalog_namespace))
...
def load_locked_package_sources(
    ...
    return tuple(sorted(package_sources, key=_package_source_namespace))
