...
class ResourceSlot(Resource):
    ...
    2. 作为原始入参（**仅框架内部解析阶段**）：见 ``ResourceSlotRawInput``，可能是
       list（一棵树的扁平节点组，或只含一棵树的 ``ResourceTreeSet.dump()`` 形态）或
       dict（资源引用，前端 schema 填写形态）。
...
#   - List[List[Dict]]：ResourceTreeSet.dump() 形态，单 ResourceSlot 时须恰好只含一棵树；
#   两种 list 形态都就地装配成一个物料（须恰好单根）。
# 解析完成后，@action 函数签名拿到的是「一个完整的 Resource 实例」（见 ResourceSlot 文档第 1 条）。
ResourceSlotRawInput = Union[
    List[Dict[str, Any]],
