...
def parse_material_source_declaration(
    ...
    *,
    ...
    # ``keywords`` 是严格八字段选择器；字段缺失、扩展或重复均不能静默兼容。
    ...
    custody_policy = _custody_policy(keywords["custody_policy"], imports=imports)
...
def render_material_source_call(
    ...
    *,
    ...
    custody_member = MATERIAL_CUSTODY_POLICY_MEMBERS[selector["custody_policy"]]
    ...
    arguments = [
        ...
        f"custody_policy=MaterialCustodyPolicy.{custody_member}",
    ]
...
def _custody_policy(
    ...
    *,
    ...
    参数说明：``expression`` 必须是显式导入的 ``MaterialCustodyPolicy`` 成员；
    ``imports`` 证明局部名身份。返回闭集 wire 值，动态值或自由字符串失败。
    """
