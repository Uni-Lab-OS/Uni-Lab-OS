...
def validate_material_graph(
    *,
    ...
    本阶段只实施物料流线性（MaterialFlowLinearity），其余参数固定深模块边界，
    不读取库存（Inventory）、物料实例或库位（Site）。返回：无；同一物料占位符
    （ResourceSlot）输出出现多条物理消费边时抛出
    ``MaterialGraphValidationError``。
    """

    # 这些事实属于后续资源模板兼容（ResourceTemplate Compatibility）与工作流
    # 边界校验；显式接收但不让本轮线性算法越权查询外部存储。
    del nodes, templates, effective_params, validated_workflow_io
