...
def _direct_parent_runtime_uuid(
    ...
    *,
    ...
    参数：当前节点与按 Graph ``id`` 索引的同代快照。返回：``parent_uuid``
    或 ``parent`` 设备 id 对应节点的 uuid。占用检测必须停在库位本身。
    """
...
def _parent_runtime_uuid(
    ...
    *,
    ...
    参数：当前节点、按 Graph ``id`` / 运行时 UUID 索引的同代快照，以及库位
    运行时身份。返回：父物料运行时 UUID；根节点为 ``None``。Graph 产品字段是
    ``parent``（设备 id），``dump()`` 会改写成 ``parent_uuid``；直接父若是
    库位则再取库位的父物料。
    """
